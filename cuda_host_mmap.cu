...
#ifdef KEPLER
const size_t max_dipole_size =3l*1024l*1024l*1024l; 
#else
const size_t max_dipole_size = 3l*1024l*1024l*1024l; 
#endif
int itransit=0;
...
__host__ void dipole_initialise_cpu(FintensityJob* intensity){
	...
	if(dipole_size > max_dipole_size){
		...
		int parts = ceil(double(dipole_size)/double(max_dipole_size));
...
__host__ void dipole_do_intensities_async_omp(FintensityJob & intensity,int device_id,int num_devices){
	...
	double** eigenvec_unit = new double*[2*intensity.molec.sym_nrepres];
	int* ieigenvec_unit = new int[2*intensity.molec.sym_nrepres];
	size_t* seigenvec_unit = new size_t[2*intensity.molec.sym_nrepres];
	for(int i =0; i< 2; i++){
		for(int j = 0; j < intensity.molec.sym_nrepres; j++)
		{
			...
			sprintf(filename,j0eigen_vector_gamma_filebase,intensity.jvals[i],j+1);
			printf("Reading %s\n",filename);
			ieigenvec_unit[i + j*2] = open(filename,O_RDONLY);
			seigenvec_unit[i + j*2] =  GetFilenameSize(filename);
			eigenvec_unit[i + j*2] = (double*)mmap(NULL,seigenvec_unit[i + j*2],PROT_READ,MAP_SHARED,ieigenvec_unit[i + j*2],0);//open(filename,O_RDONLY);
			madvise(eigenvec_unit[i+j*2],seigenvec_unit[i +j*2],MADV_SEQUENTIAL);
			if(eigenvec_unit[i + j*2] == MAP_FAILED)
			{
				printf("error opening %s \n",filename);
				exit(0);
			}
	...
	for(int i =0; i< 2; i++){
		for(int j = 0; j < intensity.molec.sym_nrepres; j++){
			 ...
			 madvise(eigenvec_unit[i +j*2],seigenvec_unit[i +j*2],MADV_SEQUENTIAL | MADV_WILLNEED);
	    ...
	    // printf("ilevelI=%i jI=%i energyI=%11.4f igammaI=%i ndegI=%i\n",ilevelI,jI,energyI,igammaI,ndegI);
	      ...
	      double* unitI = eigenvec_unit[ indI + (igammaI)*2]; 
		//Check filters
		...
		//If success then read
	     ...
	     // memcpy(unitI+irecI*nsizeI,initial_vector,sizeof(double)*nsizeI);
	      stat = cublasSetVector(nsizeI, sizeof(double),unitI+(irecI*nsizeI), 1, gpu_initial_vector, 1);
		if(unitI + (irecI*nsizeI) != unitI){
			madvise(unitI,(irecI-1l)*nsizeI*sizeof(double),MADV_DONTNEED);
		}
               int idegF_t = 0;
		...
		while(vector_idx < no_final_states_gpu && ilevelF < intensity.Neigenlevels)
		{
		  ...
		  //    !
		   //   !
			...
			double* unitF = eigenvec_unit[ indF + (igammaF)*2]; 			
			...
			//printf("HERE %i\n",irec*nsizeF);
			memcpy(final_vectors + vector_idx*((unsigned long int)(intensity.nsizemax)),unitF+irec*nsizeF,sizeof(double)*nsizeF);
			madvise(unitF,irec*nsizeF*sizeof(double), MADV_DONTNEED);
			//printf("THERE %i\n",irec*nsizeF);			
			//Increment
			vector_idx++;
		}
...
*/
		...
		printf("IO: \tRequests: %d\tTotal_time:%.fs\tTime per call:%12.6fs\n",io_calls,io_total_time/1000.0,io_total_time/(1000.0*double(io_calls)));
	...
	//for(int i =0; i< 2; i++)
	//	for(int j = 0; j < intensity.molec.sym_nrepres; j++)
	//		munmap(eigenvec_unit[i + j*2],seigenvec_unit[i + j*2]);
